...
项目功能宽度和领域深度已经超过典型开源原型，尤其 ChatBI 权限链、知识引用、个人工作台、场景安装和多引擎分发具备产品差异化。整改分支已建立并跑绿后端、前端、供应链、Docker Buildx 与 Trivy Quality Gates 基线，浏览器、MCP、SQL 出网、凭据、会话和工作区文件对象授权等高优先级风险也已完成首轮收敛；但项目尚未达到可直接承载多租户生产流量的“企业级”标准。运维上仍是单实例、单 Redis、进程内调度，并缺少 SLO、备份恢复与故障演练。
