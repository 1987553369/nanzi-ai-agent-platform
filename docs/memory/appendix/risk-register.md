...
| SEC-06 | P1（应用 HTTP/SMTP 路径已缓解） | MCP 和其他可配置目标存在 SSRF/重绑定风险 | 公网 HTTP 与 SMTP 路径已固定解析；受控私网要求集成专属 Host + CIDR，HTTP 凭据绑定 Origin，SMTP 增加端口审批和强制 TLS；继续覆盖数据库原生协议、模型 SDK、Browser Worker 和网络层 egress | Redirect、Rebinding、IPv4/IPv6/元数据、私网审批、跨 Origin 与跨协议测试 |
