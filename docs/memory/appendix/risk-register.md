...
| SEC-06 | P1（应用 HTTP/SMTP/数据库路径已缓解） | MCP 和其他可配置目标存在 SSRF/重绑定风险 | 公网 HTTP、SMTP 与数据库原生协议已固定解析；受控私网要求 Host + CIDR，SMTP/数据库增加端口审批；继续覆盖模型 SDK、Browser Worker 和网络层 egress | Redirect、Rebinding、IPv4/IPv6/元数据、私网审批、跨 Origin 与跨协议测试 |
| SEC-06A | P1（已缓解） | 历史 MCP 凭据被回显、明文保存和写入日志 | 已完成 DTO 分离、版本化密文、存量隔离迁移和日志脱敏；继续建设 KMS 轮换 | API/状态机契约、双数据库迁移清单、日志脱敏检查 |
| SEC-06B | P0（应用层已缓解） | 外部数据源历史密码曾明文存储并由响应模型回传前端 | 已完成版本化密文、V118/V17 存量隔离、默认 dry-run 离线迁移、响应状态化、运行时明文拒绝和轮换销毁旧池；继续接入 KMS | 数据源凭据/迁移/前端负向契约已通过；待真实环境备份、dry-run、迁移和密钥轮换演练 |
| SEC-06C | P0（应用层已缓解） | 外部数据库连接曾允许明文或驱动默认 TLS 降级 | 已统一五类驱动 TLS 策略、审批目录 CA、V119/V18 字段和生产强制开关；固定 IP 驱动只声明 CA 验证，不虚报身份校验 | 离线 TLS/路径/迁移/前端契约已通过；待隔离环境真实驱动、证书过期/吊销/轮换和 Oracle Wallet 演练 |
...
| ENG-01 | P1（部分缓解） | Python 运行依赖未完整锁定，历史类型/Lint/漏洞债务缺少强制门禁 | 已拆 Runtime/Dev、固定开发工具和 Docker 补丁版本，新增增量 Ruff/SAST、测试/构建、Secret Scan、依赖审计、双 SBOM 与镜像扫描；继续生成 hash lock、固定 Digest、清零报告并转硬阻断 | 本地 CI 契约/自检通过；待远端首次跑绿、可重复干净构建和扫描报告清零 |
